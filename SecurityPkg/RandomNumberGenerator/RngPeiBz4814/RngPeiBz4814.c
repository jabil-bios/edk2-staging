...
#include <Ppi/Bz4814Rng.h>

/**
  Returns information about the random number generation implementation.

  @param[in]     This                 A pointer to the BZ4814_RNG_PPI instance.
  ...
  )
{
  ...
  if (RngAlgorithm != NULL) {
    ...
    Status        = Bz4814RngGetInfo (This, &AlgorithmSize, &SupportedAlgorithm);
    if (EFI_ERROR (Status)) {
  ...
  if ((RngAlgorithm == NULL) || CompareGuid (RngAlgorithm, &SupportedAlgorithm)) {
    Status = Bz4814RngGetBytes (RngValueLength, RngValue);
    return Status;
  }
